...
def report(
    ...
    top_k_categories: int = typer.Option(5, "--top-k-categories", help="Сколько top-значений выводить для категориальных признаков."),
    title: str = typer.Option("EDA-отчёт", "--title", help="Заголовок отчёта (# ... в report.md)."),
    min_missing_share: float = typer.Option(0.3, "--min-missing-share", help="Порог доли пропусков для списка проблемных колонок."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Есть числовые колонки с долей нулей > {quality_flags['zero_share_threshold']:.0%}: **{quality_flags['has_many_zero_values']}**\n\n")
        ...
        if not problematic_missing:
            f.write("Проблемных колонок не найдено.\n\n")
        else:
            for col, share in sorted(problematic_missing.items(), key=lambda x: x[1], reverse=True):
        ...
        if not top_cats:
            ...
            f.write(f"См. файлы в папке `top_categories/` (top_k={top_k_categories}).\n\n")
