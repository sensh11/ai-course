...
def compute_quality_flags(
    ...
    *,
    ...
    constant_columns: List[str] = [
        c.name for c in summary.columns if int(c.unique) <= 1
    ]
    flags["has_constant_columns"] = len(constant_columns) > 0
    flags["constant_columns"] = constant_columns

    zero_share_by_col: Dict[str, float] = {}
    if df is not None and not df.empty:
        numeric_cols = [c.name for c in summary.columns if c.is_numeric]
        for col in numeric_cols:
            try:
                s = pd.to_numeric(df[col], errors="coerce")
                denom = float(s.notna().mean())
                if denom == 0.0:
                    zero_share_by_col[col] = 0.0
                else:
                    zero_share_by_col[col] = float((s == 0).mean())
            except Exception:
                continue
    flags["zero_share_by_col"] = zero_share_by_col
    flags["has_many_zero_values"] = any(
        share > zero_share_threshold for share in zero_share_by_col.values()
    )
